...
def apply_mean_shift_fast(image, window_size, convergence_threshold, sample_fraction=0.01):
    ...
    # Step 1: Randomly sample a fraction of pixels
    ...
    # Step 2: Apply mean-shift only on sampled pixels
    ...
    for i in range(len(sampled_pixels)):
        ...
        while np.linalg.norm(current_mean - prev_mean) > convergence_threshold:
            prev_mean = current_mean.copy()
            distances = np.linalg.norm(sampled_pixels - current_mean, axis=1)
    ...
    # Step 3: Cluster sampled pixels
    modes_rounded = np.round(modes, decimals=1)
    unique_modes, inverse_indices = np.unique(modes_rounded, axis=0, return_inverse=True)
    
    # Step 4: Assign every pixel in original image to nearest cluster
    result = np.zeros_like(pixels)
    for i in range(num_pixels):
        distances = np.linalg.norm(unique_modes - pixels[i], axis=1)
